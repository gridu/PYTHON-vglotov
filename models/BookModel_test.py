...
def test_book_addition():
    ...
    Book.add_book(Book('Satire', title, '2019-11-11', None))
...
def test_book_deletion():
    ...
    Book.add_book(Book('Satire', title, '2019-11-11', None))
    assert Book.get_book_by_title(title).__len__() == 1
    assert Book.delete_book((Book.get_book_by_title(title)[0])['id'])
...
def test_book_title_update():
    ...
    assert Book.delete_book((Book.get_book_by_title(new_title)[0])['id'])
...
def test_limited_amount_of_latest_books():
    title = 'Feathers'
    Book.add_book(Book('Drama', title, '2019-12-11', None))
    Book.add_book(Book('Satire', title, '2019-10-11', None))
    Book.add_book(Book('Romance', title, '2019-09-11', None))
...
def test_book_info_getting():
    ...
    Book.add_book(Book('Drama', title, '2019-12-04', None))
    book_id = (Book.get_book_by_title(title)[0])['id']
    Book.rename_book(book_id, new_title)
...
def test_get_all_ids_by_title():
    ...
    Book.add_book(Book('Drama', title, '2019-12-11', None))
    Book.add_book(Book('Satire', title, '2019-10-11', None))
    Book.add_book(Book('Romance', 'Butterfly', '2019-09-11', None))
...
def test_all_books_getting():
    ...
    title = 'Mother of wind' + datetime.now().isoformat()
    Book.add_book(Book('Satire', title, '2019-11-03', None))
    id_to_delete.append((Book.get_book_by_title(title)[0])['id'])
